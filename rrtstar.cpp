...
#define  PI 3.14;
const float goalb = 0.1;
const float step= 0.1 ;
const float hood=0.13;
const float step_cost=0.05;
using namespace std;
...
// }
// RRT planner
void RRTpath(std::vector<float> start,std::vector<float>goal)
{
  // initializations
  ...
  tree.number_nodes();
  std::vector<std::vector<float> > path;
  std::vector<float> samp,dir, low(3,0),high(3,20);
  unsigned int i,j;
  bool goalr;
